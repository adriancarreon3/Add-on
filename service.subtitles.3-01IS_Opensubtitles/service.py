...
from OSUtilities import OSDBServer, log, hashFile, normalizeString_opensubtitles
...
def Search_opensubtitles( item ):
  ...
  if search_data != None:
    search_data.sort(key=lambda x: [not x['MatchedBy'] == 'moviehash',
				     ...
				     not normalizeString_opensubtitles(xbmc.getInfoLabel("VideoPlayer.OriginalTitle")).lower() in x['SubFileName'].replace('.',' ').lower(),
...
def Download_opensubtitles(id,url,format,stack=False):
  subtitle_list = []
...
if params['action'] == 'search' or params['action'] == 'manualsearch':
  ...
  item['tvshow']             = normalizeString_opensubtitles(xbmc.getInfoLabel("VideoPlayer.TVshowtitle"))  # Show
  item['title']              = normalizeString_opensubtitles(xbmc.getInfoLabel("VideoPlayer.OriginalTitle"))# try to get original title
  ...
  if item['title'] == "":
    ...
    item['title']  = normalizeString_opensubtitles(xbmc.getInfoLabel("VideoPlayer.Title"))      # no original title, get just Title
  ...
  Search_opensubtitles(item)

elif params['action'] == 'download':
  subs = Download_opensubtitles(params["ID"], params["link"],params["format"])
  for sub in subs:
