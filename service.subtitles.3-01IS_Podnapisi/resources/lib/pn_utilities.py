...
def TraducirLenguaje(lang, lang_from, lang_to):
  for x in LANGUAGES:
    if lang == x[lang_from] :
      return x[lang_to]
...
def invertir(basestring):
  asal = [basestring[i:i+2]
          for i in range(0, len(basestring), 2)]
  asal.reverse()
  return ''.join(asal)

def calcular_SublightHash(filename):
  ...
  hash = hash + invertir(dec2hex(begining, 8))
  ...
  hash = hash + invertir(dec2hex(middle, 8))
  ...
  hash = hash + invertir(dec2hex(end, 8))
...
class PNServer:
  ...
  def Buscar_SubitulosWeb( self, item):
  ...
  def comparar_columnas(self, b, a):
    return cmp( b["language_name"], a["language_name"] )  or cmp( a["sync"], b["sync"] ) 

  def unir_subtitulos(self):
    if( len ( self.subtitles_list ) > 0 ):
      self.subtitles_list = sorted(self.subtitles_list, self.comparar_columnas)
